...
@section('title', 'Edit Description - Creavibe')

@section('content')
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <a href="{{ route('blog-admin.descriptions.index') }}" class="text-gray-400 hover:text-brand-accent transition">
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form action="{{ route('blog-admin.descriptions.update', $description->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $description->title) }}" required
                       ...
                       placeholder="e.g. About Creavibe">
                @error('title')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="8"
                          class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-brand-accent focus:border-brand-accent transition outline-none"
                          placeholder="Write the description content...">{{ old('content', $description->content) }}</textarea>
                <p class="mt-1 text-xs text-gray-500">Use the editor to format text with headings, lists, bold, italic, etc.</p>
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $description->order) }}"
                       class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-brand-accent focus:border-brand-accent transition outline-none"
                       placeholder="0">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $description->is_active) ? 'checked' : '' }}
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                <a href="{{ route('blog-admin.descriptions.index') }}" class="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    Cancel
                </a>
