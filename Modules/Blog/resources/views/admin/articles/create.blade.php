...
@section('title', 'Write Article - Creavibe Editorial')

@section('content')
@php
    $showOnBlog = session()->hasOldInput() ? old('show_on_blog', false) : true;
    $isTrending = $showOnBlog && old('is_trending', false);
@endphp
<div class="mb-6 flex items-center justify-between">
    ...
    <div class="flex gap-3">
        ...
        <button type="submit" form="article-create-form" onclick="document.getElementById('status').value = 'Draft'" class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none transition">
            Save Draft
        </button>
        <button type="submit" form="article-create-form" onclick="document.getElementById('status').value = 'Published'" class="bg-brand-accent border border-transparent rounded-md shadow-sm py-2 px-6 inline-flex justify-center text-sm font-medium text-white hover:bg-[#d66c2e] focus:outline-none transition">
            Publish
        </button>
    </div>
</div>

<form id="article-create-form" action="{{ route('blog-admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row gap-6">
    @csrf
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden p-6">
            ...
            <div class="mb-6">
                ...
                <div class="mt-1 flex rounded-md shadow-sm">
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" class="block w-full rounded-none rounded-r-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-accent focus:outline-none focus:ring-brand-accent dark:border-gray-600 dark:bg-gray-700 dark:text-white" placeholder="seo-friendly-url-slug" data-initial-slug="{{ old('slug') }}">
                </div>
                <p id="slug-help" class="mt-1 text-xs text-gray-500 dark:text-gray-400">Auto-generated from the headline. You can edit it manually.</p>
            </div>

            <div class="mb-6">
                <label for="excerpt" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Excerpt (Brief Summary)</label>
                <textarea id="excerpt" name="excerpt" rows="3" class="shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-md focus:outline-none p-3 placeholder-gray-400" placeholder="A short summary of the article..."></textarea>
            </div>
    ...
    <div class="w-full lg:w-80 flex-shrink-0 space-y-6">
                ...
                <div>
                    ...
                    <select id="category_id" name="blog_category_id" required class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none appearance-none">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" @selected((int) old('blog_category_id') === $category->id)>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="author_id" name="author_id" required class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none appearance-none">
                        ...
                        @foreach($authors as $author)
                            ...
                            @endphp
                            <option value="{{ $author->id }}" @selected((int) old('author_id') === $author->id) data-author="{{ json_encode($authorData, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) }}">{{ $author->name }}{{ $author->designation ? ' - '.$author->designation : '' }}</option>
                        @endforeach
                    </select>
                    @error('author_id')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div class="space-y-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                    <label class="flex items-start gap-2">
                        ...
                        <span>
                            ...
                            <span class="block text-xs text-gray-500 dark:text-gray-400">If disabled, this article is hidden from /blog and opens from /slug for SEO landing pages.</span>
                        </span>
                    </label>
                    <label class="flex items-start gap-2">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-4 space-y-4">
                <div>
                    <label for="image_file" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Upload Image</label>
                ...
                <div>
                    ...
                    <input type="text" id="image" name="image" value="{{ old('image') }}" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="https://...">
                </div>
                <div class="border-t border-gray-200 pt-4 dark:border-gray-700">
                    ...
                    <div class="mt-3 space-y-4">
                        <div>
                            ...
                            <input type="text" id="image_title" name="image_title" value="{{ old('image_title') }}" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="Descriptive image title">
                        </div>
                        <div>
                            <label for="image_alt_text" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Image Alt Text</label>
                            <input type="text" id="image_alt_text" name="image_alt_text" value="{{ old('image_alt_text') }}" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="Describe the image for search and accessibility">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <textarea id="image_caption" name="image_caption" rows="2" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="Optional visible caption if used later">{{ old('image_caption') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <input type="text" id="meta_title" name="meta_title" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="Optimized title...">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="meta_keywords" name="meta_keywords" class="mt-1 shadow-sm focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none" placeholder="digital pr, seo, publishers">
                </div>
            </div>
        </div>
...
<script>
    ...
    setupArticlePreview({ formSelector: '#article-create-form' });
