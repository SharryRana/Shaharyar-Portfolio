@extends('frontend.layouts.master')
@section('title', '404  Page Not Found | Creavibe')
@section('robots', 'noindex, nofollow')
@section('main-content')
<main id="main-content" style="min-height:70vh;display:flex;align-items:center;">
    <div class="container text-center" style="padding:80px 0;">
        <div class="skill-icon" style="font-size:5rem;margin-bottom:24px;">
            <i class="fas fa-circle-exclamation" aria-hidden="true"></i>
        </div>
        <h1 style="font-size:clamp(3rem,8vw,6rem);margin-bottom:8px;">404</h1>
        <h2 style="margin-top:0;">Page Not Found</h2>
        <p style="max-width:480px;margin:0 auto 32px;">
            The page you're looking for doesn't exist or has been moved.
        </p>
        <div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap;">
            <a href="{{ route('home') }}" class="btn">Go Home</a>
            <a href="{{ route('projects.index') }}" class="btn btn-outline">View Projects</a>
            <a href="{{ route('contact') }}" class="btn btn-outline">Contact Us</a>
        </div>
    </div>
</main>
@endsection
